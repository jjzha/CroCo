...
#SBATCH --job-name=dpo_training_array
#SBATCH --output=/scratch/<YOUR_PROJECT>/2026-reward/.cache/dpo_logs/dpo_%A_%a.out
#SBATCH --error=/scratch/<YOUR_PROJECT>/2026-reward/.cache/dpo_logs/dpo_%A_%a.err
#SBATCH --array=0-5
...
FILENAME=${DATASET##*/}
CURRENT_OUTPUT_DIR="/scratch/<YOUR_PROJECT>/2026-reward/reward-filtering/output/20260309-results/tmp-models-lora/dpo_low_resource/${SAFE_NAME_MODEL}-${FILENAME}-dpo-lr${LEARNING_RATE}"
...
ROCR_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 singularity exec $BIND_MOUNTS "$CONTAINER" bash -lc "
  ...
  python -m accelerate.commands.launch \
    --config_file=configs/accelerate_hf_trainer_config_dpo.yaml \
    src/dpo.py \
    --use_lora True \
    --seed 42 \
